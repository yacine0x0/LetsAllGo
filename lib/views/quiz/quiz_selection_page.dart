...
class _QuizSelectionPageState extends State<QuizSelectionPage> {
  ...
  List<Map<String, String>> get _currentChapters => _selectedAlgo == 1 ? _chaptersAlgo1 : _chaptersAlgo2;

  List<int> get _currentIntensities => _selectedAlgo == 1 ? [10, 15, 20, 30] : [5, 10, 15, 20];
  ...
  Future<void> _startQuiz() async {
    ...
    try {
      if (_selectedAlgo == 1) {
        ...
        if (!mounted) return;
        Navigator.push(context, MaterialPageRoute(builder: (_) => QuizPageContent(controller: controller, algoType: 'algo1')));
      } else {
        ...
        if (!mounted) return;
        Navigator.push(context, MaterialPageRoute(builder: (_) => QuizPageContent(controller: controller, algoType: 'algo2')));
      }
    } catch (e) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(_lang.t('Erreur lors du chargement du quiz', 'Error loading quiz')),
      ...
      child: Column(
        children: [
          ...
          GestureDetector(
            ...
            child: _buildSidebarItem(icon: Icons.logout, label: _lang.t("Déconnexion", "Logout"), h: h, isLogout: true),
          ),
          SizedBox(height: h * 0.02),
        ],
      ),
  ...
  void _navigateToPage(int index) {
    if (index == 0) Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => const DashboardPage()));
    else if (index == 2) Navigator.push(context, MaterialPageRoute(builder: (_) => const LeaderboardPage()));
    else if (index == 3) Navigator.push(context, MaterialPageRoute(builder: (_) => const FilesPage()));
  }
  ...
  Widget _buildSidebarItem({
    ...
    return Row(
      children: [
        Container(width: 4, height: h * 0.08, decoration: BoxDecoration(color: isActive ? Colors.blue : Colors.transparent, borderRadius: BorderRadius.circular(2))),
        Expanded(
          child: Padding(
            padding: EdgeInsets.symmetric(vertical: h * 0.02),
            child: Column(
              children: [
                Icon(icon, color: isActive ? Colors.blue : (isLogout ? Colors.red : Colors.white70), size: h * 0.06),
                SizedBox(height: h * 0.005),
                Text(label, style: TextStyle(color: isActive ? Colors.blue : (isLogout ? Colors.red : Colors.white70), fontSize: h * 0.020, fontWeight: isActive ? FontWeight.bold : FontWeight.normal)),
              ],
            ),
          ),
        ),
  ...
  Widget _buildHeader(double h) {
    return Column(
      ...
      children: [
        ...
        Text(
          _selectedAlgo == 1
              ? _lang.t("Sélectionne les chapitres et le niveau de difficulté", "Select chapters and quiz intensity to begin")
              : _lang.t("Structures de données avancées — Sélectionne les chapitres et la difficulté", "Advanced data structures — Select chapters and difficulty"),
  ...
  Widget _buildFilters(double h) {
    return Row(
      children: [
        GestureDetector(onTap: () => _switchAlgo(1), child: _filterChip(_lang.t("Algo 1", "Algo 1"), h, selected: _selectedAlgo == 1)),
        const SizedBox(width: 8),
        GestureDetector(onTap: () => _switchAlgo(2), child: _filterChip(_lang.t("Algo 2", "Algo 2"), h, selected: _selectedAlgo == 2)),
      ],
    );
  }
  ...
  Widget _filterChip(String label, double h, {bool selected = false}) {
    return Container(
      ...
      child: Text(label, style: TextStyle(color: Colors.white, fontSize: h * 0.015, fontWeight: selected ? FontWeight.bold : FontWeight.normal)),
    );
  }

  // ==================== PROGRESS STEPS ====================
  Widget _buildProgressSteps(double h, double w) {
    final steps = [_lang.t("Chapitres", "Chapters"), _lang.t("Difficulté", "Intensity"), _lang.t("Résumé", "Review")];
    return Row(
      children: List.generate(steps.length * 2 - 1, (i) {
        if (i.isOdd) return Expanded(child: Container(height: 2, color: _currentStep > (i ~/ 2) + 1 ? Colors.blue : Colors.white24));
          ...
          children: [
            Container(
              ...
              decoration: BoxDecoration(
                ...
                color: isDone ? Colors.green : (isActive ? Colors.blue : Colors.white24),
                border: Border.all(color: isActive ? Colors.blue : Colors.white24, width: 2),
              ),
              child: Center(
                child: isDone
                    ? Icon(Icons.check, color: Colors.white, size: h * 0.025)
                    : Text("$stepIndex", style: TextStyle(color: isActive ? Colors.white : Colors.white38, fontSize: h * 0.022, fontWeight: FontWeight.bold)),
              ),
            ),
            SizedBox(height: h * 0.008),
            Text(steps[i ~/ 2], style: TextStyle(color: isActive ? Colors.blue : Colors.white38, fontSize: h * 0.014, fontWeight: isActive ? FontWeight.bold : FontWeight.normal)),
          ],
        );
      }),
    );
      ...
      children: [
        ...
        Expanded(
          child: GridView.builder(
            gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 2, crossAxisSpacing: 60, mainAxisSpacing: 25, childAspectRatio: 3.5),
            ...
            itemBuilder: (context, index) {
              ...
              return GestureDetector(
                onTap: () {
                  setState(() {
                    if (isSelected) _selectedChapters.remove(chapter["id"]);
                    else _selectedChapters.add(chapter["id"]!);
                  });
                },
                child: ClipRRect(
                  borderRadius: const BorderRadius.only(topLeft: Radius.circular(13), topRight: Radius.circular(13), bottomLeft: Radius.circular(27), bottomRight: Radius.circular(27)),
                  child: BackdropFilter(
                    filter: ImageFilter.blur(sigmaX: 10, sigmaY: 10),
                    child: Container(
                      padding: EdgeInsets.only(top: h * 0.005, left: h * 0.02, right: h * 0.02, bottom: h * 0.02),
                      decoration: BoxDecoration(
                        color: isSelected ? Colors.blue.withValues(alpha: 0.3) : Colors.white.withValues(alpha: 0.2),
                      ...
                      child: Row(
                        children: [
                          Expanded(
                            child: FittedBox(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(chapter["id"]!, style: TextStyle(color: Colors.blue, fontSize: h * 0.025, fontWeight: FontWeight.bold)),
                                  SizedBox(height: h * 0.005),
                                  Text(chapter["title"]!, style: TextStyle(color: Colors.white, fontSize: h * 0.02, fontWeight: FontWeight.bold), maxLines: 2, overflow: TextOverflow.ellipsis),
                                ],
                              ),
                            ),
                          ),
                          ...
                          Stack(
                            ...
                            children: [
                              Image.asset(chapter["icon"]!, width: h * 0.08, height: h * 0.08),
                              Container(
                                width: h * 0.025,
                                height: h * 0.025,
                                decoration: BoxDecoration(shape: BoxShape.circle, color: isSelected ? Colors.blue : Colors.white24, border: Border.all(color: isSelected ? Colors.blue : Colors.white54, width: 2)),
                                child: isSelected ? Icon(Icons.check, color: Colors.white, size: h * 0.015) : null,
                              ),
                            ],
                          ),
                        ],
  ...
  // ==================== INTENSITÉ (RESTaurée complètement) ====================
  Widget _buildIntensitySelection(double h, double w) {
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(_lang.t("Niveau de difficulté du quiz", "Quiz difficulty level"),
            style: TextStyle(color: Colors.white, fontSize: h * 0.022, fontWeight: FontWeight.bold)),
        SizedBox(height: h * 0.02),
        Wrap(
          spacing: 16,
                ...
                child: Column(
                  children: [
                    Text(_currentIntensityLabels[index],
                        style: TextStyle(color: isSelected ? color : Colors.white70, fontSize: h * 0.020, fontWeight: isSelected ? FontWeight.bold : FontWeight.normal)),
                    Text("$intensity ${_lang.t("questions par chapitre", "questions/chapter")}",
                        style: TextStyle(color: isSelected ? color.withValues(alpha: 0.8) : Colors.white54, fontSize: h * 0.012)),
                  ],
                ),
              ),
            );
        ...
        Container(
          ...
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(_lang.t("Que signifie le niveau de difficulté ?", "What does difficulty level mean?"),
              ...
              children: [
                Row(
                  children: [
                    ...
                    Text(_lang.t("Configuration du Quiz", "Quiz Configuration") + " — ${_selectedAlgo == 1 ? "Algo 1" : "Algo 2"}",
                        style: TextStyle(color: Colors.white, fontSize: h * 0.022, fontWeight: FontWeight.bold)),
                  ],
                ),
                SizedBox(height: h * 0.02),
                _buildReviewRow(h, _lang.t("Chapitres sélectionnés", "Selected Chapters"), "${_selectedChapters.length}"),
                SizedBox(height: h * 0.02),
                _buildReviewRow(h, _lang.t("Niveau de difficulté", "Difficulty Level"), "${_currentIntensityLabels[_currentIntensities.indexOf(_selectedIntensity)]} ($_selectedIntensity ${_lang.t("questions par chapitre", "questions/chapter")})"),
                SizedBox(height: h * 0.02),
                _buildReviewRow(h, _lang.t("Total de questions", "Total Questions"), "$totalQuestions"),
                SizedBox(height: h * 0.02),
                Container(
                  ...
                  child: Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          _lang.t("Tu es prêt à commencer ! Clique sur 'Commencer le Quiz'", "You're ready to start! Click 'Start Quiz' to begin."),
          ...
          if (_currentStep > 1)
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.12), borderRadius: BorderRadius.circular(10), border: Border.all(color: Colors.white30)),
                child: Row(
                  children: [
                    Icon(Icons.chevron_left, color: Colors.white, size: h * 0.02),
                    Text(_lang.t("Retour", "Back"), style: TextStyle(color: Colors.white, fontSize: h * 0.018)),
                  ],
                ),
              ),
            )
          ...
          if (_currentStep < 3)
            GestureDetector(
              onTap: () {
                if (_currentStep == 1 && _selectedChapters.isEmpty) {
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text(_lang.t("Veuillez sélectionner au moins un chapitre", "Please select at least one chapter")), backgroundColor: Colors.orange.shade700),
                  );
                  return;
                }
              ...
              child: Container(
                ...
                child: Row(
                  children: [
                    Text(_lang.t("Suivant", "Next"), style: TextStyle(color: Colors.white, fontSize: h * 0.018, fontWeight: FontWeight.bold)),
                    const SizedBox(width: 8),
            ...
            GestureDetector(
              ...
              child: Container(
                ...
                decoration: BoxDecoration(
                  gradient: LinearGradient(colors: _isLoadingQuiz ? [Colors.grey, Colors.grey.shade700] : [Colors.green, Colors.green.shade700]),
                  borderRadius: BorderRadius.circular(10),
                ),
                child: Row(
                  children: [
                    if (_isLoadingQuiz)
                      SizedBox(width: h * 0.022, height: h * 0.022, child: const CircularProgressIndicator(color: Colors.white, strokeWidth: 2))
                    else
                      Icon(Icons.play_arrow, color: Colors.white, size: h * 0.022),
                    const SizedBox(width: 8),
                    Text(_isLoadingQuiz ? _lang.t("Chargement...", "Loading...") : _lang.t("Commencer le Quiz", "Start Quiz"),
  ...
  Widget _buildRightPanel(double h, double w) {
    ...
    return ClipRRect(
      child: BackdropFilter(
        ...
        child: Container(
          width: w * 0.22,
          decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.06), border: Border(left: BorderSide(color: Colors.blue.withValues(alpha: 0.2)))),
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  Text(_lang.t("Infos Quiz", "Quiz Info") + " — ${_selectedAlgo == 1 ? "Algo 1" : "Algo 2"}",
                      style: TextStyle(color: Colors.white, fontSize: h * 0.020, fontWeight: FontWeight.bold)),
                ],
              ),
              SizedBox(height: h * 0.02),
              _buildInfoCard(h, icon: Icons.school, label: _lang.t("Total chapitres", "Total Chapters"), value: "$totalChapters", color: Colors.blue),
              SizedBox(height: h * 0.015),
              _buildInfoCard(h, icon: Icons.check_circle_outline, label: _lang.t("Sélectionnés", "Selected"), value: "${_selectedChapters.length}", color: Colors.green),
              SizedBox(height: h * 0.015),
              _buildInfoCard(h, icon: Icons.assignment, label: _lang.t("Questions", "Questions"), value: "$totalQuestions", color: Colors.purple),
            ],
          ),
        ),
      ),
  ...
  Widget _buildInfoCard(double h, {required IconData icon, required String label, required String value, required Color color}) {
    return Container(
